...
export function TeacherMessages() {
  ...
  const loadChat = async (studentId: string) => {
    try {
      const studentProfile = await profiles.get(studentId)
      setSelectedStudent(studentProfile || null)
      const chatData = await messages.getConversation(user!.id, studentId)
      setChatMessages(chatData)

      // Mark unread messages as read
      for (const msg of chatData) {
        if (msg.receiver_id === user?.id && !msg.is_read) {
          await messages.markAsRead(msg.id)
        }
      }
    } catch (error) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setNewMessage('')
      // Reload chat
      const chatData = await messages.getConversation(user!.id, selectedStudent.id)
      setChatMessages(chatData)
    } catch (error) {
  ...
  // Get unique students from conversations
  const uniqueStudents = Array.from(
    new Map(
      conversations.map((msg) => {
        const studentId = msg.sender_id === user?.id ? msg.receiver_id : msg.sender_id
        const student = msg.sender_id === user?.id ? msg.receiver : msg.sender
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-gray-500">Chat with your students directly.</p>
      </div>
            ...
            {uniqueStudents.length === 0 ? (
              ...
              uniqueStudents.map((student) => {
                const lastMessage = conversations.find(
                  (msg) =>
                    (msg.sender_id === user?.id && msg.receiver_id === student.id) ||
                    (msg.sender_id === student.id && msg.receiver_id === user?.id)
                )
                return (
                  <button
                    key={student.id}
                    onClick={() => loadChat(student.id)}
