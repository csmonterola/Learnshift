...
export function StudentSettings() {
  ...
  const handleSaveProfile = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    try {
      await studentApi.updateProfile({ name, email, avatar, grade_level: gradeLevel, section })
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    try {
      await studentApi.updatePassword({ current_password: currentPassword, password: newPassword, password_confirmation: confirmPassword })
      setCurrentPassword('')
      setNewPassword('')
      setConfirmPassword('')
  ...
  const handleSavePreferences = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    try {
      await studentApi.updatePreferences({ timezone, language, email_notifications: emailNotifications, theme })
        ...
        <div className="flex-1">
          {activeTab === 'general' && (
            <form onSubmit={handleSaveProfile} className="bg-white rounded-2xl border border-gray-200 p-6 space-y-6">
              ...
              {/* Grade Level */}
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">Grade Level</label>
                <input
                  type="text"
                  value={gradeLevel}
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-0.5">Receive email updates about your progress and assignments.</p>
                    </div>
                  </div>
                  <button
